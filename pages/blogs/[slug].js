        ...
        <main className="mt-10 dark:bg-dark dark:text-light">
                    ...
                    <div className="space-x-2">
                        {
                            post?.categories && post?.categories?.map((category, idx) => {
                                return (
                                    <div
                                        key={idx}
                                        className="px-4 py-1 bg-black text-gray-200 inline-flex items-center justify-center mb-2">{category?.title}</div>
                                )
                            })
                        }
                    </div>
...

            {
                submitted ?
                    ...
                    :
                    <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col p-5 max-w-2xl mx-auto border border-brand-500'>
                        ...
                        <label className='block mb-5'>
                            ...
                            <textarea {...register("comment", { required: true })} className='shadow border rounded py-2 px-3 form-textarea mt-1 block w-full outline-none' placeholder='Your Name' rows={8} name="comment" id="comment" />
                        </label>
                        ...
                        <input type="submit" className='bg-brand-500 shadow hover:bg-brand-400 focus:shadow-outline focus:outline-none text-white font-bold rounded cursor-pointer py-2 px-4' />
                    </form>
            }
                    ...
                    post?.comments && post?.comments.map((comment) => {
                        return (
                            <div className="" key={comment._id}>
                                <p><span className='text-brand-500 font-semibold'>{comment.name}:</span> {comment.comment}</p>
                            </div>
                        )
                    })
                }
