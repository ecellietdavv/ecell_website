...
const Bootcamp23 = () => {
    ...
    const onSubmit = async data => {
        ...
        await fetch('/api/register', {
            method: "POST",
            body: JSON.stringify({ ...data, startup: startup }),
        }).then(() => {
    ...
    useEffect(() => {
        if (formState.isSubmitSuccessful) {
            reset({ name: '', email: '', profession: '', collage: '', startup: startup, stage: '' });
        }
    }, [formState, reset]);
        ...
        <div className="grid min-h-screen grid-cols-1 gap-8 px-8 py-16 justify-center items-center mx-auto rounded-lg md:grid-cols-2 md:px-12 lg:px-16 xl:px-32 dark:bg-dark dark:text-gray-100">
            <div className="flex flex-col justify-between">
                <div className="space-y-2">
                    <h2 className="text-4xl font-bold leading-tight lg:text-5xl">Entrepreneurship Bootcamp 23</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 ng-untouched ng-pristine ng-valid">
                <div>
                    ...
                    <input {...register("name", { required: true })} type="text" placeholder="John Doe" className="w-full p-3 rounded dark:bg-mid" />
                </div>
                <div>
                    <label className="text-sm">Email</label>
                    <input {...register("email", { required: true })} type="email" className="w-full p-3 rounded dark:bg-mid" />
                </div>
                <div>
                    <label className="text-sm">Profession</label>
                    <input {...register("profession", { required: true })} type="text" className="w-full p-3 rounded dark:bg-mid" />
                </div>
                <div>
                    <label className="text-sm">Collage</label>
                    <input {...register("collage", { required: true })} type="text" className="w-full p-3 rounded dark:bg-mid" />
                </div>
                <div>
                    ...
                    <div className="flex items-center space-x-6">
                        <div className="flex items-center space-x-2">
                            <p>Yes</p>
                            <input type="radio" name='Yes' id='yes' onClick={() => { setStartup(true) }} className="w-full p-3 rounded dark:bg-mid" />
                        </div>
                        <div className="flex items-center space-x-2">
                            <p>No</p>
                            <input type="radio" name='No' id='no' onClick={() => { setStartup(false) }} className="w-full p-3 rounded dark:bg-mid" />
                        </div>
                    </div>
                </div>
                {
                    startup ? <div>
                        ...
                        <input {...register("stage", { required: true })} type="text" className="w-full p-3 rounded dark:bg-mid" />
                    </div>
                        : null
                }
